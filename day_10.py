...
sample = """.F-7F-..|-F-|F---7J7FLF-7F|FJ-LJ.7--F7-|77.|7J.FFLF7-FJ7.J-7-|.J.7.F7L7..FF.|F--F--|J-FFJFL7F7-LJFF-.7F|7.FF|7FF7FL7L--F--77-7F7.7.-F-J.JFLF
...
for y in sample.splitlines():
    grid.append(list('-'.join(y)))
    grid.append(["|"]*len(grid[0]))
    colors.append([0] * len(grid[0]))
    colors.append([0] * len(grid[0]))
        ...
        if c == "S":
            ...
            while c != "S":     #loop until we're back
                ...
                directions = [(1, 0),  #0 east
                              ...
                              (-1, 0),  #2 west
                              (0, -1)]  #3 north
                xa = x + directions[dir][0]
                ya = y + directions[dir][1]
...
with Image.new("RGBA", (len(grid[0]), len(grid)), (255, 255, 255, 255)) as im:
    ...
    ImageDraw.floodfill(im, (0, 0), (0, 255, 0, 255), thresh=50)
    nonWhitePixels=[]
    for i in range(0, imageSizeW,2):
        for j in range(0, imageSizeH,2):
            pixVal = im.getpixel((i, j))
            if pixVal[2] > 200:
                im.putpixel((i, j),(0, 0, 255, 255))
                nonWhitePixels.append([i, j])
sum_b=len(nonWhitePixels)
im.save("day10.png", "PNG")
print("part A:", sum_a)
print("part B:", sum_b)
